import paramiko,os, boto3, json, requests,base64,pysnow,logging,inspect,warnings,substring
...
def lambda_handler(event, context):
    ...
    processPidsOutput = MainMethod(EventIpAddress, "ps -eo pcpu,pid,user,args | sort -k 1 -r | head -6")

    '''   Status of the Method is to get the top five process names   '''
    processNamesOutput = MainMethod(EventIpAddress, "ps -eo cmd --sort=-%cpu | head -n 6").replace('CMD', '')

    '''   Status of the Method is to get VitalStatsInfo cpuory    '''
    VitalStatsOutput = MainMethod(EventIpAddress, "vmstat 1 10")

    '''   Status of the Method is to get the cpu consumption   '''
    cpuConsumptionOutput = MainMethod(EventIpAddress, "iostat -xk | head -n 20")
    ...
    MemInfo = MainMethod(EventIpAddress, "ps -eo pmem,pcpu,vsize,pid,cmd | sort -k 1 -nr | head -5")
    
    '''Comments to be printed'''

    LinuxVer = "The linux version is: " + '\r\n ' + LinuxVersion
    ProcessPid = "The processes consuming more CPU is: " + '\r\n' + processPidsOutput
    Top5 = "The top five processes consuming more cpu is: " + '\r\n' + processNamesOutput
    VitalInfo = "The vital statistics information is: " + '\r\n ' + VitalStatsOutput
    cpuInfo = "The CPU consumption information is : " + '\r\n' + cpuConsumptionOutput
    ...
    Top5Mem =  "The top 5 processes consuming memory is: " + '\r\n' +  MemInfo
